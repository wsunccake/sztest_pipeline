...
mkdir -p $VAR_DIR/output/l3acp

NEW_INPUT=partner_domain_l3acp.inp
INPUT_NUMBER=1000
TMP_DIR=`mktemp -d`
...
for domain_name in `cat $VAR_DIR/input/partner_domains/domains.inp`; do
  ...
  # create l3acp
  for name in `cat $VAR_DIR/input/l3acp/$domain_name.inp`; do
    if [ ! -z $domain_id ]; then
      echo "domain: $domain_name $domain_id l3acp: $name" >> $TMP_DIR/$NEW_INPUT
    fi
  done
done

split -l $INPUT_NUMBER $TMP_DIR/$NEW_INPUT $TMP_DIR/in_
cp -fv $TMP_DIR/$NEW_INPUT $VAR_DIR/input/l3acp/.
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # create l3acp
  cat $f | xargs -n5 -P $NPROC sh -c 'create_l3acp ${4} ${2} | tee ${VAR_DIR}/output/l3acp/${1}_${4}.out'
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def result = util.checkResponseStatus "${VAR_DIR}/output/l3acp"
                    println result
        ...
        stage('Statistic Response') {
            steps {
                script {
                    util.statisticizeResponse "${VAR_DIR}/output/l3acp"
                }
            }
        }
