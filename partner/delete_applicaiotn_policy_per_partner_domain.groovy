...
mkdir -p $VAR_DIR/output/delete_application_policy
split -l $INPUT_NUMBER $VAR_DIR/output/id/application_policy_v2_ids.log $TMP_DIR/in_
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  sed 's/|/ /g' $f | xargs -n2 -P $NPROC sh -c 'delete_application_policy ${0} | tee ${VAR_DIR}/output/delete_application_policy/${0}.out'
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def result = util.checkResponseStatus "${VAR_DIR}/output/delete_application_policy"
                    println result
        ...
        stage('Statistic Response') {
            steps {
                script {
                    util.statisticizeResponse "${VAR_DIR}/output/delete_application_policy"
                }
            }
        }
