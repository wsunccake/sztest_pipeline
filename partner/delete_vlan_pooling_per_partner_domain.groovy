...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # delete vlan pooling
  sed 's/|/ /g' $f | xargs -n2 -P $NPROC sh -c 'delete_vlan_pooling ${0} | tee ${VAR_DIR}/output/delete_vlan_pooling/${0}.out'
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def result = util.checkResponseStatus "${VAR_DIR}/output/delete_vlan_pooling", "204"
                    println result
        ...
        stage('Statistic Response') {
            steps {
                script {
                    util.statisticizeResponse "${VAR_DIR}/output/delete_vlan_pooling", "204"
                }
            }
        }
