...
split -l $INPUT_NUMBER $VAR_DIR/output/id/wifi_calling_ids.log $TMP_DIR/in_
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # delete wifi calling
  sed 's/|/ /g' $f | xargs -n2 -P $NPROC sh -c 'delete_wifi_calling ${0} | tee ${VAR_DIR}/output/delete_wifi_calling/${0}.out'
