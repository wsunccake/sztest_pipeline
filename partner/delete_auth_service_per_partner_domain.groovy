...
mkdir -p $VAR_DIR/output/delete_auth_service
split -l $INPUT_NUMBER $VAR_DIR/output/id/proxy_auth_ids.log $TMP_DIR/in_
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # delete proxy auth
  sed 's/|/ /g' $f | xargs -n2 -P $NPROC sh -c 'delete_auth_service ${0} | tee ${VAR_DIR}/output/delete_auth_service/${0}.out'
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def result = util.checkResponseStatus "${VAR_DIR}/output/delete_auth_service"
                    println result
        ...
        stage('Statistic Response') {
            steps {
                script {
                    util.statisticizeResponse "${VAR_DIR}/output/delete_auth_service"
                }
            }
        }
