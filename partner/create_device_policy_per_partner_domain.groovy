...
mkdir -p $VAR_DIR/output/device_policy

NEW_INPUT=partner_domain_device_policy.inp
INPUT_NUMBER=1000
TMP_DIR=`mktemp -d`
...
for domain_name in `cat $VAR_DIR/input/partner_domains/domains.inp`; do
  ...
  for name in `cat $VAR_DIR/input/device_policy/$domain_name.inp`; do
    if [ ! -z $domain_id ]; then
      echo "domain: $domain_name $domain_id device_policy: $name" >> $TMP_DIR/$NEW_INPUT
    fi
  done
done

split -l $INPUT_NUMBER $TMP_DIR/$NEW_INPUT $TMP_DIR/in_
cp -fv $TMP_DIR/$NEW_INPUT $VAR_DIR/input/device_policy/.
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # create device_policy
  cat $f | xargs -n5 -P $NPROC sh -c 'create_domain_device_policy ${4} ${2} | tee ${VAR_DIR}/output/device_policy/${1}_${4}.out'
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def result = util.checkResponseStatus "${VAR_DIR}/output/device_policy"
                    println result
        ...
        stage('Statistic Response') {
            steps {
                script {
                    util.statisticizeResponse "${VAR_DIR}/output/device_policy"
                }
            }
        }
