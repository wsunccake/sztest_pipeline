...
mkdir -p $VAR_DIR/output/delete_l2acl
split -l $INPUT_NUMBER $VAR_DIR/output/id/l2acl_ids.log $TMP_DIR/in_
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # delete l2acl
  sed 's/|/ /g' $f | xargs -n2 -P $NPROC sh -c 'delete_l2acl ${0} | tee ${VAR_DIR}/output/delete_l2acl/${0}.out'
...
'''
        ...
        stage('Check Response') {
            steps {
                script {
                    def result = util.checkResponseStatus "${VAR_DIR}/output/delete_l2acl"
                    println result
        ...
        stage('Statistic Response') {
            steps {
                script {
                    util.statisticizeResponse "${VAR_DIR}/output/delete_l2acl"
                }
            }
        }
