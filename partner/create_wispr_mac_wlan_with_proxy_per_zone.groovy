...
NEW_INPUT=zone_wispr_mac_wlan.inp
INPUT_NUMBER=1000
TMP_DIR=`mktemp -d`
...
for domain_name in `cat $VAR_DIR/input/partner_domains/domains.inp`; do
  ...
  if [ ! -z $domain_id ]; then
    for zone_name in `cat $VAR_DIR/input/zones/$domain_name.inp`; do
      ...
      if [ ! -z $zone_id ]; then      
        for name in `grep wisprmac $VAR_DIR/input/wlans/$zone_name.inp`; do
          n=1
...
for f in `ls $TMP_DIR/in_*`; do
  ...
  # create wlan
  cat $f | xargs -n11 -P $NPROC sh -c 'create_wispr_mac_wlan_with_proxy ${4} ${2} ${6} ${8} ${10} | tee ${VAR_DIR}/output/wispr_mac_wlans/${1}_${4}.out'
